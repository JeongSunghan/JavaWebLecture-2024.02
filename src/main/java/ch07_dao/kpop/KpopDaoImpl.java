...
public class KpopDaoImpl implements KpopDao {
	...
	public List<Kpop> getKpopList() {
		Connection conn = getConnection();
		String sql = "SELECT g.*, s.title, s.lyrics FROM girl_group g" + "  JOIN song s ON g.hit_song_id=s.sid"
				+ "  ORDER BY debut desc";
		...
		try {
			...
			while (rs.next()) {
				Kpop kpop = new Kpop(rs.getInt(1), rs.getString(2), LocalDate.parse(rs.getString(3)), rs.getInt(4),
