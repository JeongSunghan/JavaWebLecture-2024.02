...
public class SkiEqDao {
	...
	public void insertEquipment(SkiEquipment equipment) {
		// 장비 추가 메서드
		Connection conn = getConnection();
		String sql = "INSERT INTO ski_equipment (equipment_id, user_id, equipment_name, description, `condition`, image_url) VALUES (?, ?, ?, ?, ?, ?)";
		try {
		    PreparedStatement pstmt = conn.prepareStatement(sql);
		    ...
		    pstmt.setString(6, equipment.getImageUrl());
	...
	public void updateEquipment(SkiEquipment equipment) {
		// 장비 업데이트 메서드
		Connection conn = getConnection();
		String sql = "UPDATE ski_equipment SET equipment_name=?, description=?, `condition`=?, image_url=? WHERE equipment_id=?";
		try {
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, equipment.getEquipmentName());
			pstmt.setString(2, equipment.getDescription());
			pstmt.setString(3, equipment.getCondition());
			pstmt.setString(4, equipment.getImageUrl());
