						...
						<tr>
							...
							<td>
								...
								<c:if test="${user.uid eq sessUid}">
									<a class="ms-2" href="/jw/bbs/user/update?uid=${user.uid}"><i class="fa-solid fa-user-pen"></i></a>
								</c:if>
								
								<c:if test="${user.uid ne sessUid}">
									<a class="ms-2" href="#" class="disabled-link"><i class="fa-solid fa-user-pen"></i></a>
								</c:if>
								<!-- 본인 또는 관리자만 삭제 가능-->
								<c:if test="${(user.uid eq sessUid) or (sessUid eq 'admin')}">
									<a class="ms-2" href="/jw/bbs/user/delete?uid=${user.uid}"><i class="fa-solid fa-user-minus"></i></a>
								</c:if>
								<c:if test="${(user.uid ne sessUid) and (sessUid ne 'admin')}">
									<a class="ms-2" href="#" class="disabled-link"><i class="fa-solid fa-user-minus"></i></a>
								</c:if>
							</td>
						</tr>
						</c:forEach>					
